...
if [ "${syncsuccessful}" == "0" ]; then
    ...
    mv lineage_guacamole.mk ${rom_vendor_name}_guacamole.mk
    sed -i "s+lineage+$rom_vendor_name+g" ${rom_vendor_name}_guacamole.mk
    ...
    [ -f $FILE ] && echo "$FILE exists, skipping." || sed -i "s+common_full_phone.mk+common.mk+g" device/oneplus/guacamole/${rom_vendor_name}_guacamole.mk
